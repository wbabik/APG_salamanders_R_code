...
tempTAPs <- function(t, categ = "raw_MIP_seg_min20cov", fr = frame, stop.rm = TRUE){
  ...
  mh <- readRDS(paste0("rds/", t, "_seg_microhap_", categ, ".rds")) %>% 
    left_join(gene_ids, by = "gene") %>% mutate(seg_hap = str_replace_all(seg_hap, "X", "N")) %>% 
    filter(gene_sym == "TAP1" | gene_sym == "TAP2") %>%  as.data.frame() 
  ...
  bg_list <- vector("list", length(segs))
...
if(file.exists("C:/Dropbox/Rfunctions/Rfunctions_WB_MHC.R")){
  source("C:/Dropbox/Rfunctions/Rfunctions_WB_MHC.R")
} else {
  source("D:/Dropbox/Rfunctions/Rfunctions_WB_MHC.R") 
}
...
b <- lapply(sp, tempTAPs)

bb <- bind_rows(b)

saveRDS(bb, "rds/All_species_aa_segments_TAP12.rds")
